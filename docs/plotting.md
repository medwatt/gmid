...
This page covers plotting quantities from a [lookup table](lookup_tables.md) and
reading values from it by interpolation. Work in a notebook or REPL: the table
is loaded once, and every query after that is instant.
...
fixes two of the four variables and keeps the other two: the primary variable is
swept along each curve, and the secondary variable gives one curve per value.
...
Here `vbs` and `vds` are fixed, `vgs` is swept (limited to the given range), and
every length in the table becomes a curve. `mos` is the model name used when the
table was built. PMOS voltages keep their sign.
...
An expression is anything computed from the table. Every `Mosfet` has these
built in:

| Expression | Quantity |
|---|---|
| `gmid_expression` | gm/ID |
| `current_density_expression` | ID/W |
| `transit_frequency_expression` | fT = gm / 2πCgg |
| `gain_expression` | gm/gds |
| `early_voltage_expression` | VA = ID/gds |
| `inverse_early_voltage_expression` | gds/ID |
| `rds_expression` | rds = 1/gds |
| `vstar_expression` | V* = 2ID/gm |
| `vov_expression` | VOV = VGS − VTH |
...
A custom expression takes the table variables it needs and a function that
combines them. For example the figure of merit fT · gm/ID:
...
The names you can use are the saved parameters
(`lookup_table["parameter_names"]`) plus `length`, `vgs`, `vds`, and `vbs`.

## Design charts: `plot_by_expression`

Plots one expression against another over the device you defined, with one curve
per secondary value. Use `filtered_values` to pick which curves to show.
...
Add `y2_expression` for a second y-axis (dashed):
...
A custom expression is used the same way:
...
`plot_by_sweep` takes a new slice: give all four variables and name the primary
one. Use it for I-V curves or anything swept over length.

Transfer characteristic (ID vs. VGS, one curve per length):
...
Output characteristic (ID vs. VDS, one curve per VGS). A `(start, stop, step)`
tuple picks evenly spaced values:
...
Speed and gain vs. length (one curve per VGS):
...
To overlay quantities that do not come from a single plot call, pull the numbers
out with `lookup_expression_from_table`, then pass them to `quick_plot`. This
reads values straight from the table grid, with no interpolation. Below are
three common definitions of the saturation voltage, for one length:
...
`interpolate` returns the value of any expression at a point given by two other
expressions, interpolating between table entries:
...
expanded like `np.arange`. Two tuples evaluate every combination and return the
results as one flat array.

## Plot options

All three plot functions accept:
