...
class LookupTableGenerator:
    def __init__(
        ...
        self.parameter_names = ['id', 'vth', 'gm', 'gmbs', 'gds', 'cgg', 'cgs', 'cbg', 'cgd', 'cdd']
        self.lookup_table = {}
    ...
    def __initalize(self):
        ...
        for p in self.parameter_names:
            self.lookup_table[self.identifier][p] = np.zeros(shape=(self.n_lengths, self.n_vsb, self.n_vds, self.n_vgs))
    ...
    def __build_netlist(self, length, vsb):
        ...
        circuit.M(self.identifier, 'ND', 'NG', circuit.gnd, 'NB', model=self.model_names[self.identifier], l=length, w=self.width)
        return circuit

    def __simulate_circuit(self, circuit, mos):
        simulator = circuit.simulator(temperature=self.temp, nominal_temperature=self.temp)
        simulator.save_internal_parameters(*[f'@{mos}[{p}]' for p in self.parameter_names])
        analysis = simulator.dc(VGS=slice(*(self.vgs*self.r)), VDS=slice(*(self.vds*self.r)))
        return analysis

    def __save_parameters(self, analysis, mos, length, vsb):
        for p in self.parameter_names:
            res = analysis[f'@{mos}[{p}]']
            self.lookup_table[self.identifier][p][length][vsb] = res.reshape(self.n_vds, self.n_vgs)
