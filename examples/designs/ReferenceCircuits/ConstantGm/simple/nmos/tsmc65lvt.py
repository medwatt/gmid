...
TARGET_SPECS = {
    "gm_ref":     Spec(150e-6, "max", 5.0),   # want gm_ref >= 150 uS (worst corner)
    "gmR":        Spec(1.0, "eq", 20.0),
    "VDS_margin": Spec(0.10, "max", 10.0),    # keep M1/M4 saturated with headroom
...
CORNERS = [
    ...
    # Corner("tt_08", f"{LUT_DIR}/tsmc_65_lv_spectre_tt.npz", NMOS, PMOS, conditions=dict(vdd=1.0)),
    # Corner("ff_08", f"{LUT_DIR}/tsmc_65_lv_spectre_ff.npz", NMOS, PMOS, conditions=dict(vdd=1.0)),
    # Corner("ss_08", f"{LUT_DIR}/tsmc_65_lv_spectre_ss.npz", NMOS, PMOS, conditions=dict(vdd=1.0)),
]
...
if __name__ == "__main__":
    run(workers=4, circuit=Circuit, parameters=PARAMETERS, target_specs=TARGET_SPECS,
        ...
        maxiter=50, seed=1, n_restarts=1)
