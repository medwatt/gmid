...
class Circuit(CircuitModel):
    ...
    MOSFETS = [
        ...
        Instance("M3a", "nmos", d="n04", g="vcascn", s="n05", b="vss"),
        Instance("M3b", "nmos", d="VOUT", g="vcascn", s="n06", b="vss"),
        Instance("M4a", "nmos", d="n05", g="n04", s="vss", b="vss"),
        Instance("M4b", "nmos", d="n06", g="n04", s="vss", b="vss"),
    ...
    VSOURCES = [
        ...
        VSource("VBP", p="vbp", n="vss", mirror="M5"),
    ]
    ...
    # re-solve the saturation margins they are derived from (M1a for the input-side cascode,
    ...
    # Every device VDS is an unknown for robustness. Many are explicit node
    ...
    UNKNOWNS = [
        ...
        Unknown(
            "Mvbp_GMID", seed=lambda c: 12.0, bound=lambda c: (4.0, 30.0)
        ),  # diode replica gm/ID [1]
    ]
    ...
    def solve_point(self, v: State, dev, cond) -> State:
        ...
        M4a = dev.nmos(gmid=v.M4a_GMID, L=v.M4a_L, vds=v.M4a_VDS, vsb=0.0)
        M5_VDS = VDD - VIN_CM - M1a.vgs
        M5 = dev.pmos(gmid=v.M5_GMID, L=v.M5_L, vds=M5_VDS, vsb=0.0)
        M2a_VSB = VDD - VIN_CM + M1a.vds_used - M1a.vgs
        M2a = dev.pmos(gmid=v.M2a_GMID, L=v.M2a_L, vds=v.M2a_VDS, vsb=M2a_VSB)
        M3a_VSB = M4a.vds_used
        M3a = dev.nmos(gmid=v.M3a_GMID, L=v.M3a_L, vds=v.M3a_VDS, vsb=M3a_VSB)
        M2b_VDS = M2a.vds_used
        M2b = dev.pmos(gmid=v.M2a_GMID, L=v.M2a_L, vds=M2b_VDS, vsb=v.M2b_VSB)
        M3b = dev.nmos(gmid=v.M3a_GMID, L=v.M3a_L, vds=v.M3b_VDS, vsb=v.M3b_VSB)
        M4b_VDS = M3a.vgs - M3b.vgs + M4a.vds_used
        M4b = dev.nmos(gmid=v.M4a_GMID, L=v.M4a_L, vds=M4b_VDS, vsb=0.0)
        M1b_VDS = M1a.vds_used + M2a.vgs - M2b.vgs
        M1b = dev.pmos(gmid=v.M1a_GMID, L=v.M1a_L, vds=M1b_VDS, vsb=0.0)
        Mvbp = dev.pmos(gmid=v.Mvbp_GMID, L=v.M5_L, vds=M5.vgs, vsb=0.0)
        ...
        W["M3b"] = W["M3a"]
        W["M4b"] = W["M4a"]
        W["Mvbp"] = W["M5"]  # 1:1 bias diode replica
        ID["Mvbp"] = ID["M5"]
        IREF_Mvbp = ID["M5"] * Mvbp.jd / M5.jd
        ...
        ss = {
            ...
            "M5": M5.small_signal(v.M5_GMID, ID["M5"], W["M5"], ptw, use_gmb=False),
        ...
        VCASCP = VIN_CM - M1a.vds_used + M1a.vgs - M2a.vgs
        VCASCN = M3a.vgs + M4a.vds_used
        VBP = VDD - M5.vgs
        return State(
            W=W,
            L=L,
    ...
    def residuals(self, b) -> list:
        return [
            ...
            vres(b.M2a_VDS, b.VIN_CM - b.M1a.vds_used + b.M1a.vgs - b.M4a.vgs),
            vres(b.M3a_VDS, -b.M4a.vds_used + b.M4a.vgs),
            vres(b.M4a_VDS, b.M4a.vdsat + b.M4a_VDSAT_MARGIN),
            vres(
                b.M2b_VSB,
                b.VDD - b.VIN_CM + b.M1a.vds_used - b.M1a.vgs + b.M2a.vgs - b.M2b.vgs,
            ),
            vres(
                b.M3b_VDS,
                b.VIN_CM
            ...
            vres(b.Mvbp.vgs, b.M5.vgs),  # diode replica shares the master's VGS
        ]

    # ---------- (7) MULTICORNER CONSERVATION ----------
    def freeze_extra(self, b) -> dict:
        return {"IREF_Mvbp": b.IREF_Mvbp, "VCASCN": b.VCASCN, "VCASCP": b.VCASCP}
    ...
    def specs(self, b, cond) -> dict:
        ...
        out["Itotal"] = Itotal
        VCASCP = b.VIN_CM - b.M1a.vds_used + b.M1a.vgs - b.M2a.vgs
        out["VCASCP"] = VCASCP
        VCASCN = b.M3a.vgs + b.M4a.vds_used
        out["VCASCN"] = VCASCN
        VBP = b.VDD - b.M5.vgs
        out["VBP"] = VBP
        VOUT_DC = b.VIN_CM - b.M1a.vds_used + b.M1a.vgs - b.M2a.vgs - b.M2b.vds_used + b.M2b.vgs
        out["VOUT_DC"] = VOUT_DC
        VOUT_MAX = -b.M2b.vdsat + b.M2b.vgs + b.VCASCP
        out["VOUT_MAX"] = VOUT_MAX
        VOUT_MIN = b.M3b.vdsat - b.M3b.vgs + b.VCASCN
        out["VOUT_MIN"] = VOUT_MIN
        VIN_MAX = -b.M1a.vgs - b.M5.vdsat + b.VDD
        out["VIN_MAX"] = VIN_MAX
        VIN_MIN = max(
            b.M1a.vdsat - b.M1a.vgs + b.M2a.vgs + b.VCASCP,
            -b.M1a.vgs + b.M1b.vdsat + b.M2b.vgs + b.VCASCP,
        )
        out["VIN_MIN"] = VIN_MIN
